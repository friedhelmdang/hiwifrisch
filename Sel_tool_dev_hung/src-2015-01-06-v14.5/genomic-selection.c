//////////////////////////////////////////////////////////////////////////
// SelectionTools main source file genomic-selection.c                   //
// (c) Matthias Frisch                                                   //
///////////////////////////////////////////////////////////////////////////
		     ...
		     int*   retval        )
{
  ...
  one_mat = malloc ( gv->NoMar * gv->NoMar * sizeof(double) );
  if ( NULL == one_mat ) ERR_M;
  for ( i = 0; i < gv->NoMar ; i++) 
    for ( j = 0; j < gv->NoMar ; j++)
      one_mat[ i*gv->NoMar + j ] = 0;
  for ( j = 0; j < gv->NoMar ; j++)
    one_mat[ j*gv->NoMar + j ] = 1;
  ...
  if ( 1 == *init_lambda )
    {
      ...
      double lambda =  ( 1 / 0.8 -1 ) * (double)gv->NoMar; 
      for ( m = 0 ; m < gv->NoMar ; m++)  
	gv->ladi[m] = lambda;
    }
...
void gs_lambda_rmlr_01 ( gs_varset_type* , int* , double* , char** ,
			 int* , int* );
...
void gs_esteff_rmlc_01 ( gs_varset_type*, int*, double*,
			 char**, int*, int*);
	...
	for (i =1; i < gv->dimu; i++)
	  {
	    ...
	    if ( 0 == strcmp (scheme,"rmlc") )
	      { 
		gs_esteff_rmlc_01(es,maxiter,precision,nixp,neinp,Sretval);
		if (-2 == *Sretval) { *retval = -2;  }
	      }

	    if ( 0 == strcmp (scheme,"rmlv") )
	      {
		gs_esteff_rmlv_01(es,maxiter,precision,nixp,neinp,Sretval);
		if (-2 == *Sretval) { *retval = -2;  }
	      }
			  ...
			  int*    retval         )
{
  ...
  if ( 0 == *nperm)
  ...
  else
    {
      ...
      gs_permtest_01 (gv,scheme,nperm,maxiter,precision,out_filename,
		      auxfiles,retval ) ;
      if (-2 == *Sretval) { *retval = -2; return; }
    }
			   ...
			   int*    retval         )
{
  ...
  gs_esteff_rmlc_01 ( gv, maxiter, precision, nixp, neinp, Sretval ) ;
  if (-2 == *Sretval) { *retval = -2; return; }
			   ...
			   int*    retval         )
{
  ...
  gs_esteff_rmlv_01 ( gv, maxiter, precision, nixp, neinp, Sretval ) ;
  if (-2 == *Sretval) { *retval = -2; return; }
   ...
   gs_FLOAT* CVest = malloc ( CVNoRuns * (1+gv->NoMar) *sizeof(gs_FLOAT) );
	 ...
	 /* --- once for each CVRun */
	 {
	   ...
	   if ( 0 == strcmp (*scheme,"rmlr") )
	     {
	        gs_esteff_rmlr_01(es,maxiter,precision,nixp,neinp,Sretval);
		if (-2 == *Sretval) { *retval = -2;  }
	     }
	   if ( 0 == strcmp (*scheme,"rmla") )
	     {
                gs_esteff_rmla_01(es,alpha,maxiter,precision,nixp,neinp,Sretval);
		if (-2 == *Sretval) { *retval = -2;  }
	     }
